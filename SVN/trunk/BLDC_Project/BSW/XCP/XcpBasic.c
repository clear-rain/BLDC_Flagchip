...
|                              ESCAN00033655: Canbedded compile error due to vuint8
...
|                              ESCAN00039240: Compile error by vuint8
...
|                              ESCAN00043113: Compiler error: identifier vuint16 not declared
...
/* Global constants with XCP Protocol Layer main and subversion */
 const vuint8 kXcpMainVersion    = (vuint8)(CP_XCP_VERSION >> 8);
 const vuint8 kXcpSubVersion     = (vuint8)(CP_XCP_VERSION & 0x00ff);
 const vuint8 kXcpReleaseVersion = (vuint8)(CP_XCP_RELEASE_VERSION);
...
vuint8 XcpReceiveFlag = 0;
vuint8 XcpReceiveBuffer[8] = {0};
/******************************************************************************
*
*
...
void Set_XcpReceiveBuffer(vuint8* data)
{
	vuint8 i;
	for(i = 0;i < 8;i++)
	{
		XcpReceiveBuffer[i] = data[i];
	}
}

vuint8 Get_XcpReceiverFlag(void)
{
	return XcpReceiveFlag;
}
...
{
	if(1 == Get_XcpReceiverFlag())
	{
		Set_XcpReceiveFlag(0);
		XcpCommand((const vuint32 *)XcpReceiveBuffer);
	}
	XcpSendCallBack();
}
...
static vuint8 XcpAllocMemory( void )
{
  ...
  s = (vuint16)( ( xcp.Daq.DaqCount      *   (vuint8)sizeof(tXcpDaqList)                           ) +
    ...
    #else
      s += 9u; /* Worst case 9 bytes needed for alignment */
    #endif
  #endif
    ...
    if ( (xcp.SessionStatus & (SessionStatusType)SS_CONNECTED) != 0 )
    {
      ...
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC32 )
        ...
        ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD44 )        
          ...
          case CC_GET_DAQ_PROCESSOR_INFO://DA
        	/* PRQA S 2003 */ /* MD_Xcp_2003 */
            {
    #if defined ( XCP_ENABLE_TESTMODE )
              if ( gDebugLevel != 0)
              {
            ...
            case CC_GET_DAQ_RESOLUTION_INFO://D9
            	/* PRQA S 2003 */ /* MD_Xcp_2003 */
              {
    #if defined ( XCP_ENABLE_TESTMODE )
                if ( gDebugLevel != 0) 
...
//              vuint8 count = 10;
          ...
          case CC_START_STOP_DAQ_LIST://DE
            {
...
/* module specific MISRA deviations:
  ...
  MD_Xcp_4130:    Rule 12.7
      ...
      Risk:       There is no risk as the target variable is intended to be vuint8.
      Prevention: Covered by code review.
*/
