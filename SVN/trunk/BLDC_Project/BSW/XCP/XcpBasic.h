...
#define DAQ_FLAG_OVERRUN                  0x0Bu /* Overun (Internal Use) */
...
#define CRM_UPLOAD_MAX_SIZE                             ((uint8)(kXcpMaxCTO-1))
...
#define CRM_SHORT_UPLOAD_MAX_SIZE                       ((uint8)(kXcpMaxCTO-1))
...
#define CRO_DOWNLOAD_MAX_SIZE                           ((uint8)(kXcpMaxCTO-2))
...
#define CRO_DOWNLOAD_NEXT_MAX_SIZE                      ((uint8)(kXcpMaxCTO-2))
...
#define CRO_DOWNLOAD_MAX_MAX_SIZE                       ((uint8)(kXcpMaxCTO-1))
...
#define CRM_SHORT_DOWNLOAD_MAX_SIZE                     ((uint8)(kXcpMaxCTO-8))
...
#define CRO_PROGRAM_MAX_SIZE                            ((uint8)(kXcpMaxCTO-2))
...
#define CRO_PROGRAM_NEXT_MAX_SIZE                       ((uint8)(kXcpMaxCTO-2))
...
#define CRO_PROGRAM_MAX_MAX_SIZE                        ((uint8)(kXcpMaxCTO-1))
...
/****************************************************************************/
  ...
  #define CRM_BYTE(x)             (xcp.Crm.b[x])
    ...
    #define CRO_WORD(x)             (((uint16)pCmd->b[((x)*2)+1] << 8) | ((uint8)pCmd->b[((x)*2)+0]))
    #define CRO_DWORD(x)            (((uint32)pCmd->b[((x)*4)+3] << 24) | ((uint32)pCmd->b[((x)*4)+2] << 16)| ((uint16)pCmd->b[((x)*4)+1] << 8)| ((uint8)pCmd->b[((x)*4)+0]))

    #define CRM_WORD(x)             (((uint16)xcp.Crm.b[((x)*2)+1] << 8) | ((uint8)xcp.Crm.b[((x)*2)+0]))
    #define CRM_WORD_WRITE(x, d)    {xcp.Crm.b[((x)*2)+0] = ((uint8)(d) & 0xFF); xcp.Crm.b[((x)*2)+1] = ((uint8)((uint16)(d) >> 8) & 0xFF);}
    #define CRM_DWORD(x)            (((uint32)xcp.Crm.b[((x)*4)+3] << 24) | ((uint32)xcp.Crm.b[((x)*4)+2] << 16) | ((uint16)xcp.Crm.b[((x)*4)+1] << 8) | ((uint8)xcp.Crm.b[((x)*4)+0]) )
    #define CRM_DWORD_WRITE(x, d)   {xcp.Crm.b[((x)*4)+0] = ((uint8)((uint32)(d) >> 0) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+1] = ((uint8)((uint32)(d) >> 8) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+2] = ((uint8)((uint32)(d) >> 16) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+3] = ((uint8)((uint32)(d) >> 24) & 0xFF);}
  #else
    #define CRO_WORD(x)             (((uint16)pCmd->b[((x)*2)+0] << 8) | ((uint8)pCmd->b[((x)*2)+1]))
    #define CRO_DWORD(x)            (((uint32)pCmd->b[((x)*4)+0] << 24) | ((uint32)pCmd->b[((x)*4)+1] << 16)| ((uint16)pCmd->b[((x)*4)+2] << 8)| ((uint8)pCmd->b[((x)*4)+3]))

    #define CRM_WORD(x)             (((uint16)xcp.Crm.b[((x)*2)+0] << 8) | ((uint8)xcp.Crm.b[((x)*2)+1]))
    #define CRM_WORD_WRITE(x, d)    {xcp.Crm.b[((x)*2)+1] = ((uint8)(d) & 0xFF); xcp.Crm.b[((x)*2)+0] = ((uint8)((uint16)(d) >> 8) & 0xFF);}
    #define CRM_DWORD(x)            (((uint32)xcp.Crm.b[((x)*4)+0] << 24) | ((uint32)xcp.Crm.b[((x)*4)+1] << 16) | ((uint16)xcp.Crm.b[((x)*4)+2] << 8) | ((uint8)xcp.Crm.b[((x)*4)+3]) )
    #define CRM_DWORD_WRITE(x, d)   {xcp.Crm.b[((x)*4)+3] = ((uint8)((uint32)(d) >> 0) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+2] = ((uint8)((uint32)(d) >> 8) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+1] = ((uint8)((uint32)(d) >> 16) & 0xFF); \
                                     xcp.Crm.b[((x)*4)+0] = ((uint8)((uint32)(d) >> 24) & 0xFF);}
  #endif
#else
...
#else
  #define DAQBYTEPTR uint8 XCP_MEMORY_FAR *
#endif

#if defined ( MTABYTEPTR )
#else
  #define MTABYTEPTR uint8 XCP_MEMORY_FAR *
#endif

/* Pointer type used to point into the xcp structure */
#if defined ( BYTEPTR )
#else
  #define BYTEPTR uint8 *
#endif

#if defined ( ROMBYTEPTR )
#else
  #define ROMBYTEPTR uint8 const *
#endif
...
typedef union { /* PRQA S 0750 */ /* MD_MSR_18.4 */
  ...
  uint8  b[ ((kXcpMaxCTO + 3) & 0xFFC)      ];
  uint16 w[ ((kXcpMaxCTO + 3) & 0xFFC) / 2  ];
  uint32 dw[ ((kXcpMaxCTO + 3) & 0xFFC) / 4 ];
} tXcpCto;
...
/****************************************************************************/
  ...
  typedef uint8 tXcpChecksumSumType;    /* Data type (width) of the actual checksum. */
  #endif
  ...
  typedef uint16 tXcpChecksumSumType;   /* Data type (width) of the actual checksum. */
  #endif
  ...
  typedef uint32 tXcpChecksumSumType;   /* Data type (width) of the actual checksum. */
  #endif
#endif
...
*******************************************************************************/
 ...
 extern const  uint8 kXcpMainVersion;
 extern const  uint8 kXcpSubVersion;
 extern const  uint8 kXcpReleaseVersion;
...
extern uint8 XcpEvent( uint8 event );
...
#else
extern void ApplXcpSend( uint8 len,const uint8* msg );
#endif
...
#else
extern MTABYTEPTR ApplXcpGetPointer( uint8 addr_ext, uint32 addr );
#endif

#if defined ( XCP_ENABLE_MEM_ACCESS_BY_APPL )
extern uint8 ApplXcpRead( uint32 addr );
extern void XCP_FAR ApplXcpWrite( uint32 addr, uint8 data );
#endif
...
extern tXcpChecksumAddType ApplXcpReadChecksumValue( uint32 addr );
    #endif
  #endif
#endif
...
#if defined ( XCP_ENABLE_CALIBRATION_MEM_ACCESS_BY_APPL ) && !defined ( XCP_ENABLE_MEM_ACCESS_BY_APPL )
extern uint8 ApplXcpCalibrationWrite(MTABYTEPTR addr, uint8 size, const BYTEPTR data);
extern uint8 ApplXcpCalibrationRead(MTABYTEPTR addr, uint8 size, BYTEPTR data);
#endif
...
void XcpSetActiveTl( uint8 MaxCto, uint8 MaxDto, uint8 ActiveTl );
uint8 XcpGetActiveTl( void );
...
extern void XcpSendEvent( uint8 evc, const BYTEPTR c, uint8 len);
#endif
...
extern void XcpPutchar( const uint8 c );
    #if defined ( XCP_ENABLE_SERV_TEXT_PRINT )
extern void XcpPrint( const uint8 *str );
      #if defined ( XCP_ENABLE_SERV_TEXT_PRINTF )
extern void XcpPrintf( const uint8 *str, ... );
      #endif
    #endif
  #endif
#endif
...
#else
extern void XcpMemCpy( DAQBYTEPTR dest, const DAQBYTEPTR src, uint8 n );
#endif
#if defined ( XcpMemSet )
#else
extern void XcpMemSet( BYTEPTR p, uint16 n, uint8 b );
#endif
...
/* Returns true (!=0) if successfull */
    ...
    #else
extern uint8 ApplXcpSendStall( void );
    #endif
  #endif
#endif
...
/* Used only, if read protection of memory areas is required */
  ...
  #else
extern uint8 ApplXcpCalculateChecksum( ROMBYTEPTR pMemArea, BYTEPTR pRes, uint32 length );
  #endif
#endif
...
extern uint8 ApplXcpGetCalPage( uint8 segment, uint8 mode ); 
extern uint8 ApplXcpSetCalPage( uint8 segment, uint8 page, uint8 mode);

/* extern void ApplXcpInitCalPage( void ); */
  #if defined ( XCP_ENABLE_PAGE_COPY )
extern uint8 ApplXcpCopyCalPage( uint8 srcSeg, uint8 srcPage, uint8 destSeg, uint8 destPage );
/* Returns:
   XCP_CMD_OK    
...
extern void ApplXcpSetFreezeMode( uint8 segment, uint8 mode );
extern uint8 ApplXcpGetFreezeMode( uint8 segment );
#endif

/* Seed & Key */
#if defined ( XCP_ENABLE_SEED_KEY )

extern uint8 ApplXcpGetSeed( const uint8 resource, uint8 *seed );
extern uint8 ApplXcpUnlock( const uint8 *key, const uint8 length );

#endif

/* User defined service of GET_ID */
#if defined ( XCP_ENABLE_GET_ID_GENERIC )
extern uint32 ApplXcpGetIdData( MTABYTEPTR *pData, uint8 id );
#endif
...
*/
extern uint8 ApplXcpUserService( const BYTEPTR pCmd );
#endif
...
extern void XcpPrintDaqList( uint8 daq );
...
void Set_XcpReceiveBuffer(uint8* data);
uint8 Get_XcpReceiverFlag(void);
void Set_XcpReceiveFlag(uint8 Flag);
