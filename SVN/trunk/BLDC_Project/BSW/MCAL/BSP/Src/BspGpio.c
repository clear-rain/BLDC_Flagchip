...
static void Bsp_HallCommonHandler(void)
{
	hall_estimator_irq();
}

static void Bsp_HallPortB_IntCallback(PORT_HandleType *pHandle, uint32_t u32pins)
{
	(void)pHandle;
	(void)u32pins;
	Bsp_HallCommonHandler();
}

static void Bsp_HallPortD_IntCallback(PORT_HandleType *pHandle, uint32_t u32pins)
{
	(void)pHandle;
	(void)u32pins;
	Bsp_HallCommonHandler();
...
static void Bsp_HallPortE_IntCallback(PORT_HandleType *pHandle, uint32_t u32pins)
{
	(void)pHandle;
	(void)u32pins;
	Bsp_HallCommonHandler();
...
void Bsp_HallExtIntInit(void)
{
	PORT_InitType tInitStruct = {0};

	/* HALL_V: IO_HALL_V_PORT / IO_HALL_V_PIN (PTD3) */
	tInitStruct.u32PortPins = IO_HALL_V_PIN;
	tInitStruct.uPortPinMux.u32PortPinMode = PORT_GPIO_MODE;
	tInitStruct.ePortGpioDir = PORT_GPIO_IN;
	tInitStruct.bPullEn = false;
	tInitStruct.ePullSel = PORT_PULL_UP;
	tInitStruct.tInterruptCfg.ePortIsrMode = PORT_IRQ_BOTH_EDGE;
	g_tPORT_D.eInstance = IO_HALL_V_PORT;
	g_tPORT_D.tSettings.PORT_PinInterruptCallBackType = Bsp_HallPortD_IntCallback;
	PORT_InitPins(&g_tPORT_D, &tInitStruct);

	/* HALL_U: IO_HALL_U_PORT / IO_HALL_U_PIN (PTB10) */
	tInitStruct.u32PortPins = IO_HALL_U_PIN;
	tInitStruct.uPortPinMux.u32PortPinMode = PORT_GPIO_MODE;
	tInitStruct.ePortGpioDir = PORT_GPIO_IN;
	tInitStruct.bPullEn = false;
	tInitStruct.ePullSel = PORT_PULL_UP;
	tInitStruct.tInterruptCfg.ePortIsrMode = PORT_IRQ_BOTH_EDGE;
	g_tPORT_B.eInstance = IO_HALL_U_PORT;
	g_tPORT_B.tSettings.PORT_PinInterruptCallBackType = Bsp_HallPortB_IntCallback;
	PORT_InitPins(&g_tPORT_B, &tInitStruct);

	/* HALL_W: IO_HALL_W_PORT / IO_HALL_W_PIN (PTE6) */
	tInitStruct.u32PortPins = IO_HALL_W_PIN;
	tInitStruct.uPortPinMux.u32PortPinMode = PORT_GPIO_MODE;
	tInitStruct.ePortGpioDir = PORT_GPIO_IN;
	tInitStruct.bPullEn = false;
	tInitStruct.ePullSel = PORT_PULL_UP;
	tInitStruct.tInterruptCfg.ePortIsrMode = PORT_IRQ_BOTH_EDGE;
	g_tPORT_E.eInstance = IO_HALL_W_PORT;
	g_tPORT_E.tSettings.PORT_PinInterruptCallBackType = Bsp_HallPortE_IntCallback;
	PORT_InitPins(&g_tPORT_E, &tInitStruct);
}
