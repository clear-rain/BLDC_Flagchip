...
#ifndef ENABLED_BLDC
/* Motor position feedback source selection. */
#define SENSOR_HALL (0U)
#define SENSOR_ENCODER (1U)
#define SENSOR_SENSORLESS (2U)

#ifndef MOTOR_SENSOR_TYPE
#define MOTOR_SENSOR_TYPE SENSOR_HALL
#endif

#if (MOTOR_SENSOR_TYPE == SENSOR_ENCODER)
#define MOTOR_SENSOR_USE_ENCODER (1U)
#else
#define MOTOR_SENSOR_USE_ENCODER (0U)
#endif

#if (MOTOR_SENSOR_TYPE == SENSOR_HALL)
#define MOTOR_SENSOR_USE_HALL (1U)
#else
#define MOTOR_SENSOR_USE_HALL (0U)
#endif
...
/* 估算器选择：
 * - 优先保持磁链分支默认 SMO_PLL；
 * - 当存在 MOTOR_SENSOR_TYPE 且显式选择编码器/霍尔时，映射到对应估算器。 */
#ifndef MOTOR_ESTIMATOR_TYPE
#if defined(MOTOR_SENSOR_TYPE) && defined(SENSOR_ENCODER) && (MOTOR_SENSOR_TYPE == SENSOR_ENCODER)
#define MOTOR_ESTIMATOR_TYPE kMotorEstimator_ENCODER
#elif defined(MOTOR_SENSOR_TYPE) && defined(SENSOR_HALL) && (MOTOR_SENSOR_TYPE == SENSOR_HALL)
#define MOTOR_ESTIMATOR_TYPE kMotorEstimator_HALL
#else
#define MOTOR_ESTIMATOR_TYPE kMotorEstimator_SMO_PLL
#endif
#endif
...
#define OPEN_LOOP_TIME_IN_SEC (float)(0.1f) //0.1
#define OPEN_LOOP_CURRENT_REF ((float)0.7f) //0.8
#endif
...
#define OPEN_LOOP_END_RPM (100)             //300
#define OPEN_LOOP_TIME_IN_SEC (float)(0.1f) //0.1
#define OPEN_LOOP_CURRENT_REF ((float)6.0f) //0.8
#endif
...
#define PI_SP_CURRENT_I (float)(0.0000005) //0.00005
#define PI_SP_CURRENT_D (float)(0.01)
...
#define PI_PLL_P (float)(35)   //16
#define PI_PLL_I (float)(0.08) //0.06
