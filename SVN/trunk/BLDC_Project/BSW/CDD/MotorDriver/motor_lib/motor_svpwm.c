...
volatile float testArrayEstimatorAngle[MOTOR_DEBUG_CAPTURE_LEN];
#endif
...
void Motor_Svpwm1(MotorAlphabetaVoltage *alpha_beta_voltage, MotorSvpwm *svpwm)
{
    const float period = (float)svpwm->period;
    const float alpha = alpha_beta_voltage->alpha;
    const float beta = alpha_beta_voltage->beta;
    ...
    ua = alpha;
    ub = -0.5f * alpha + 0.8660254f * beta;
    uc = -0.5f * alpha - 0.8660254f * beta;

    u_min = fminf(fminf(ua, ub), uc);
    u_max = fmaxf(fmaxf(ua, ub), uc);
    ...
    const float out_a = ua * period;
    const float out_b = ub * period;
    const float out_c = uc * period;
...
void Motor_Svpwm2(MotorAlphabetaVoltage *alpha_beta_voltage, MotorSvpwm *svpwm)
{
    const float period = (float)svpwm->period;
    const uint32_t period_u32 = svpwm->period;
    ...
    const float kInvSqrt3 = 0.57735026919f;
    ...
    ua = alpha * kInvSqrt3;
    ub = (-0.5f * alpha + 0.8660254f * beta) * kInvSqrt3;
    uc = (-0.5f * alpha - 0.8660254f * beta) * kInvSqrt3;
    ...
    if (u_range > 1.0f)
    {
        const float scale = 1.0f / u_range;
    ...
    svpwm->pwm_a = Motor_ClampCastPwmCount(da * period, period_u32);
    svpwm->pwm_b = Motor_ClampCastPwmCount(db * period, period_u32);
    svpwm->pwm_c = Motor_ClampCastPwmCount(dc * period, period_u32);
}
...
void Motor_TcUpdate(MotorSvpwm *svm, void *foc_str)
{
    ...
    if (testIProducerCnt < MOTOR_DEBUG_CAPTURE_LEN)
    {
        ...
        testArrayEstimatorAngle[testIProducerCnt] = ((MotorFocStruct *)foc_str)->motor_estimator.angle;
