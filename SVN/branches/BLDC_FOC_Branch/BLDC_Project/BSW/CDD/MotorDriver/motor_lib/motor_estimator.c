...
static void Motor_Hall_Transducer(MotorRotor *rotor, MotorEstimator *estimator)
{
    /* 占位：当前工程未对接霍尔采样，保持零输出防止误用 */
    estimator->angle = rotor->hall.rho;
    estimator->speed = rotor->hall.we;
}
                     ...
                     MotorEstimator *estimator)
{
    switch (estimator->type)
    {
        ...
        case kMotorEstimator_HALL:
            Motor_Hall_Transducer(rotor, estimator);
            break;
