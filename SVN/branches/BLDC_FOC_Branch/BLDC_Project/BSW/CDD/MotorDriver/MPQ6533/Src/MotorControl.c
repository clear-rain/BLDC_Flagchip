...
extern volatile MotorFocStruct motor_foc_struct;
...
//	Bsp_PWM_DutySet(e_PWM_INC_IDX, 2250);
	...
	MotorFocStruct *foc_str;
	foc_str = &motor_foc_struct;

	static uint16_t MotorTestCnt = 0u;
	MotorTestCnt++;
	if(MotorTestCnt > 200u)
	{
		MotorTestCnt = 0u;
	}

	foc_str->motor_position.angle = (float)MotorTestCnt/200.f*2.f*3.1415926f;

	foc_str->motor_d_q_voltage_out.daxis = 0.1f;
	foc_str->motor_d_q_voltage_out.qaxis = 0.f;
