#ifndef _DEVICE_FEATURES_FC4150F1M__S_FLASH_COMMON_H_
#define _DEVICE_FEATURES_FC4150F1M__S_FLASH_COMMON_H_
/*********************** FLASH Features ****************************/

#define FLASHDRV_ADDR            0x01810400U

#define DFLASH_SUPPORT                            STD_ON

#define PFLASH_FINE_SUPPORT                       STD_ON

#define FLASH_TYPE_SUPPORT                        STD_ON

#define FLASH_TYPE                                0x0
...
/** PFLASH Erase size */
#define PFLASH_ERASE_SECTOR_SIZE                  0x400U /* 1024 bytes */
#define DFLASH_PROGRAM_PAGE_MIN_SIZE              0x08U
/** DFLASH Erase size */
#define DFLASH_ERASE_SECTOR_SIZE                  0x400U /* 1024 bytes */
/** Flash start address */
...
typedef enum
{
    FLASH_ARRAY_INTEGRITY_SEQ1 = 0x0UL,
    FLASH_ARRAY_INTEGRITY_SEQ2 = 0x1UL,
    FLASH_NO_ARRAY_INTEGRITY_SEQ = 0xFFFFFFFFUL
} FLASH_ARRAY_INTEGRITY_SEQ_SELECT_T;

typedef enum
{
    FLASH_TEST_CFG_DISABLE = 0x0UL,
    FLASH_TEST_CFG_ENABLE  = 0x1UL,
    FLASH_TEST_CFG_HOLD    = 0x2UL,
...
typedef struct
{
    FLASH_DRV_TEST_CFG_E	safer;
    FLASH_DRV_TEST_CFG_E	ifr_err_clr;
    FLASH_DRV_TEST_CFG_E    stop_ack_en;
	FLASH_DRV_TEST_CFG_E    dbd;
	FLASH_DRV_TEST_CFG_E    dbd_stop_en;
	FLASH_DRV_TEST_CFG_E    fcm_cg;
	FLASH_DRV_TEST_CFG_E    sbc_cg;

}FLASH_DRV_CTL_CFG_T;

typedef uint32_t (* FLASH_ERASE_BLOCK_FUNC) (uint32_t blk_sel, uint8_t type);
typedef uint32_t (* FLASH_VOID_FUNC) (void);
typedef uint32_t (* FLASH_ERASE_SECTOR_FUNC) (FLASH_DRV_ERASESECTOR_CFG_T * flash_api_cfg, uint8_t type);
typedef uint32_t (* FLASH_ERASE_NVR) (uint32_t addr);
typedef uint32_t (* FLASH_PROGRAM_FUNC) (FLASH_DRV_PRGM_CFG_T * flash_api_cfg, uint8_t type);
typedef uint32_t (* FLASH_PROGRAM_NVR_FUNC) (FLASH_DRV_PRGM_CFG_T * flash_api_cfg);
typedef uint32_t (* FLASH_PROGRAM_NVR)  (FLASH_DRV_PRGM_CFG_T * flash_api_cfg);
...
typedef uint32_t (* FLASH_ECC_CHECK_FUNC) (FLASH_DRV_ECC_CHECK_CFG_T * flash_api_cfg, uint8_t type);
...
typedef uint32_t (* FLASH_SECTOR_PROGRAM_FUNC) (uint32_t *dest,uint32_t *addr,uint32_t *info, uint8_t type);
typedef uint32_t (* FLASH_CTL_CFG_FUNC) (FLASH_DRV_CTL_CFG_T * flash_api_cfg);
...
typedef struct
{
    ...
    FLASH_CTL_CFG_FUNC                      FLASH_DRV_CTL_CFG_FUNC;
